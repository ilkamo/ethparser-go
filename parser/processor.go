...
// parser repositories are idempotent. It is an all or nothing approach that works well if the num
func (p *Parser) processBlocks(ctx context.Context) error {
	ctx, cancel := context.WithTimeout(ctx, defaultBlocksProcessTimeout)
	defer cancel()

	blocksToProcess, lastBlockOfTheSequence, err := p.getNumberOfBlocksToProcess(ctx)
	if err != nil {
	...
	for i := 0; i < blocksToProcess; i++ {
		...
		go func(blockNumber uint64) {
			...
			block, err := p.ethClient.GetBlockByNumber(ctx, blockNumber)
			if err != nil {
		...
		}(p.lastProcessedBlock + uint64(i) + 1)
	}
	wg.Wait()
	...
	if err = p.transactionsRepo.SaveLastProcessedBlock(ctx, lastBlockOfTheSequence); err != nil {
		return fmt.Errorf("could not save last processed block of the sequence: %w", err)
	}

	// Move the sequence forward.
	p.setLastProcessedBlock(lastBlockOfTheSequence)
